...
static const char* const kAccessKey = "14ac4b88b2eddb816d19b5be066205ea4a6cb6cf";
static const char* const kSecretKey = "e3398582a7de5329e8da96429c479e4452462ed4";
...
- (void) onVuforiaUpdate: (Vuforia::State *) state {
    ...
    else if (statusCode == Vuforia::TargetFinder::UPDATE_RESULTS_AVAILABLE)
    {
        ...
        if(finder->getResultCount() > 0)
        {
            ...
            const Vuforia::TargetSearchResult* result = finder->getResult(0);
            
             NSLog(@"The result is %s",result->getMetaData());
            
            NSString *profDataString = [[NSString alloc] initWithUTF8String:result->getMetaData()];
           profDataString =[profDataString stringByReplacingOccurrencesOfString:@"'" withString:@"\""];
            NSData *data = [profDataString dataUsingEncoding:NSUTF8StringEncoding];
...
//            RestAPITest * restAPITest = [[RestAPITest alloc] init];
            NSString *profileId = [[NSString alloc] init];
            NSString *profileIdType = [[NSString alloc] init];
            
            @try {
                profileId =[profData objectForKey:@"data_id"];
                profileIdType = [profData objectForKey:@"id_type"];
            }
            @catch (NSException *exception) {
                NSLog(@"The NSDICtionary exception is %@", exception.reason);
            }
            
            
            
//            [restAPITest printProfessorData:profileId];
            [self performSegueWithIdentifier:@"OpenDashboard"
                                      sender:profData];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([segue isKindOfClass:[PresentMenuSegue class]]) {
        ...
        dashboardViewController.profileData = sender;
    }
}
...
- (void) scanlineStart {
    ...
    if (scanLineView) {
        ...
        // replace the 60 with the toolBar height
        animation.toValue = [NSValue valueWithCGPoint:CGPointMake(scanLineView.center.x, frame.size.height-60)];
